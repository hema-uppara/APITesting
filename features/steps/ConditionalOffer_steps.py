...
@given("the Submit Conditional Offer API is available")# type: ignore
def step_api_available(context):
    context.url = f"{context.base_url}{context.endpoint}"


@when('I submit a Conditional offer using "{payload_file}"')# type: ignore
...
@then("the response status code should be {expected_status:d}")# type: ignore
...
@then("the response should contain {field}")# type: ignore
