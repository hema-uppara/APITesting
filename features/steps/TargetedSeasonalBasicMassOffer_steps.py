...
@given(u'the Submit TargetedSeasonalBasicMassOffer API is available')# type: ignore
def step_api_available(context):
    context.url = f"{context.base_url}{context.endpoint}"


@when('I submit a TargetedSeasonalBasicMassOffer offer using "{payload_file}"')# type: ignore
...
@then("the TargetedSeasonalBasicMassOffer response status code should be {expected_status:d}")# type: ignore
...
@then("the TargetedSeasonalBasicMassOffer response should contain {field}")# type: ignore
...
@then('the TargetedSeasonalBasicMassOffer response status should be "{expected_status}"')# type: ignore
