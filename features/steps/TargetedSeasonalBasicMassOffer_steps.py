...
@given(u'the Submit TargetedSeasonalBasicMassOffer API is available')
def step_api_available(context):
    context.url = f"{context.base_url}{context.endpoint}"


@when('I submit a TargetedSeasonalBasicMassOffer offer using "{payload_file}"')
...
@then("the TargetedSeasonalBasicMassOffer response status code should be {expected_status:d}")
...
@then("the TargetedSeasonalBasicMassOffer response should contain {field}")
...
@then('the TargetedSeasonalBasicMassOffer response status should be "{expected_status}"')
