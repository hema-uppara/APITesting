...
@given(u'the Submit GameMassOffer Offer API is available')# type: ignore
def step_api_available(context):
    context.url = f"{context.base_url}{context.endpoint}"


@when('I submit a GameMass offer using "{payload_file}"')# type: ignore
...
@then("the response GameMass status code should be {expected_status:d}")# type: ignore
...
@then("the response GameMass should contain {field}")# type: ignore
...
@then('the response GameMass status should be "{expected_status}"')# type: ignore
