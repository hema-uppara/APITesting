...
@when('I submit a TargetedGameMass offer using "{payload_file}"')
...
@then("the TargetedGameMass response status code should be {expected_status:d}")
...
@then("the TargetedGameMass response should contain {field}")
...
@then('the TargetedGameMass response status should be "{expected_status}"')
