...
@given("the Submit WelcomeOffer API is available")# type: ignore
def step_api_available(context):
    context.url = f"{context.base_url}{context.endpoint}"


@when('I submit a WelcomeOffer using "{payload_file}"')# type: ignore
...
@then("the WelcomeOffer response status code should be {expected_status:d}")# type: ignore
...
@then("the WelcomeOffer response should contain {field}")# type: ignore
...
@then('the WelcomeOffer response status should be "{expected_status}"')# type: ignore
