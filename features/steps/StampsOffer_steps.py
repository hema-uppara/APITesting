...
@given(u'the Submit StampsOffer API is available')
def step_api_available(context):
    context.url = f"{context.base_url}{context.endpoint}"


@when('I submit a StampsOffer offer using "{payload_file}"')
...
@then("the StampsOffer response status code should be {expected_status:d}")
...
@then("the StampsOffer response should contain {field}")
...
@then('the StampsOffer response status should be "{expected_status}"')
