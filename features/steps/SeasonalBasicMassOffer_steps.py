...
@given(u'the Submit SeasonalBasicMassOffer API is available')
def step_api_available(context):
    context.url = f"{context.base_url}{context.endpoint}"


@when('I submit a SeasonalBasicMassOffer offer using "{payload_file}"')
...
@then("the SeasonalBasicMassOffer response status code should be {expected_status:d}")
...
@then("the SeasonalBasicMassOffer response should contain {field}")
...
@then('the SeasonalBasicMassOffer response status should be "{expected_status}"')
