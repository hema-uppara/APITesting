...
@given("the Submit PartnerSurprise Offer API is available")# type: ignore
def step_api_available(context):
    context.url = f"{context.base_url}{context.endpoint}"


@when('I submit a PartnerSurprise offer using "{payload_file}"')# type: ignore
...
@then("the PartnerSurprise response status code should be {expected_status:d}")# type: ignore
...
@then("the PartnerSurprise response should contain {field}")# type: ignore
...
@then('the PartnerSurprise response status should be "{expected_status}"')# type: ignore
