...
@given(u'the Submit TestTechTargetGroupOffer API is available')# type: ignore
def step_api_available(context):
    context.url = f"{context.base_url}{context.endpoint}"


@when('I submit a TestTechTargetGroupOffer offer using "{payload_file}"')# type: ignore
...
@then("the TestTechTargetGroupOffer response status code should be {expected_status:d}")# type: ignore
...
@then("the TestTechTargetGroupOffer response should contain {field}")# type: ignore
...
@then('the TestTechTargetGroupOffer response status should be "{expected_status}"')# type: ignore
