...
@given(u'the Submit TechTargetGroupOffer API is available')# type: ignore
def step_api_available(context):
    context.url = f"{context.base_url}{context.endpoint}"


@when('I submit a TechTargetGroupOffer offer using "{payload_file}"')# type: ignore
...
@then("the TechTargetGroupOffer response status code should be {expected_status:d}")# type: ignore
...
@then("the TechTargetGroupOffer response should contain {field}")# type: ignore
...
@then('the TechTargetGroupOffer response status should be "{expected_status}"')# type: ignore
