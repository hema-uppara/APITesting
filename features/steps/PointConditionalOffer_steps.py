...
@given(u'the Submit PointConditionalOffer API is available')
def step_api_available(context):
    context.url = f"{context.base_url}{context.endpoint}"


@when('I submit a PointConditionalOffer offer using "{payload_file}"')
...
@then("the PointConditionalOffer response status code should be {expected_status:d}")
...
@then("the PointConditionalOffer response should contain {field}")
...
@then('the PointConditionalOffer response status should be "{expected_status}"')
